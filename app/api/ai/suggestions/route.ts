...
export async function POST(request: NextRequest) {
  try {
    const { connectionId, fullSchema, apiKey, selectedTable } =
      await request.json();
    ...
    // PRIORITY 1: Get query history-based suggestions
    const historySuggestions = getHistoryBasedSuggestions(
      connectionId,
      selectedTable?.table
    );

    // If we have enough history, return those suggestions immediately
    if (historySuggestions.length >= 8) {
      return NextResponse.json({
        suggestions: historySuggestions.slice(0, 8),
        source: "history",
      });
    }
    ...
    if (rateLimit) {
      if (now < rateLimit.resetTime) {
        if (rateLimit.count >= MAX_REQUESTS_PER_HOUR) {
          // If rate limited, combine history with fallback
    ...
    const tableNames = fullSchema
      .flatMap(
        (s: { schema_name: string; tables: Array<{ table_name: string }> }) =>
          s.tables.map(
            (t: { table_name: string }) => `${s.schema_name}.${t.table_name}`
          )
      )
      .sort();
    const cacheKey = `${connectionId}-${tableNames.join(",")}-${
      selectedTable?.schema || ""
    }.${selectedTable?.table || ""}`;

    // Check cache for AI suggestions
    const cached = suggestionCache.get(cacheKey);
    if (cached && now - cached.timestamp < CACHE_TTL) {
      // Combine cached AI suggestions with history
      const combined = [...historySuggestions, ...cached.suggestions]
    ...
    // If no API key, combine history with smart fallback
    if (!apiKey) {
      const fallbackSuggestions = generateFallbackSuggestions(
        fullSchema,
        selectedTable
      );
      const combined = [...historySuggestions, ...fallbackSuggestions]
    ...
    const simplifiedTables = fullSchema
    ...
    const focusTable = selectedTable
      ...
      : simplifiedTables[0]?.name;
    const suggestionPrompt = `Generate 8 short query suggestions for database with tables: ${simplifiedTables
      .map((t) => t.name)
      .join(", ")}.
Focus on: ${focusTable}
Return JSON array only: ["suggestion1", "suggestion2", ...]`;
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        aiSuggestions = parsed.slice(0, 8);
      } else {
    ...
    if (aiSuggestions.length === 0) {
      aiSuggestions = generateFallbackSuggestions(fullSchema, selectedTable);
    }

    // PRIORITY: Combine history suggestions with AI suggestions (history first)
    ...
    return NextResponse.json({
      suggestions: combined,
      source: historySuggestions.length > 0 ? "history+ai" : "ai",
    });
  } catch (error: unknown) {
    console.error("Error generating suggestions:", error);

    // Try to get history-based suggestions as fallback
    const { connectionId, selectedTable } = await request
      .json()
      .catch(() => ({}));
    const historySuggestions = connectionId
      ? getHistoryBasedSuggestions(connectionId, selectedTable?.table)
      : [];

    if (historySuggestions.length >= 5) {
      return NextResponse.json({
        suggestions: historySuggestions.slice(0, 8),
        source: "history",
      });
 ...
 * Returns titles/descriptions of most frequently executed queries
 */
function getHistoryBasedSuggestions(
  ...
  try {
    // Get most frequent queries for this connection/table
    const frequentQueries = QueryHistoryManager.getMostFrequentQueries(
      connectionId,
      tableName,
      10 // Get top 10
    );

    // Convert to suggestion strings (use title if available, otherwise generate one)
    return frequentQueries.map(
      (query) =>
        query.title || QueryHistoryManager.generateSimpleTitle(query.sql)
    );
  } catch (error) {
...
function generateFallbackSuggestions(
  ...
  const focusTable = selectedTable
    ? allTables.find(
        (t) =>
          t.name === selectedTable.table && t.schema === selectedTable.schema
      )
    : allTables[0];
  ...
  if (focusTable) {
    ...
    const dateColumns = focusTable.columns.filter(
      (col: FallbackColumn) =>
    ...
    const nameColumns = focusTable.columns.filter(
      (col: FallbackColumn) =>
        col.column_name.includes("name") ||
    ...
    const idColumns = focusTable.columns.filter(
      (col: FallbackColumn & { is_primary_key?: boolean }) =>
        col.column_name.includes("id") && !col.is_primary_key
    );

    if (dateColumns.length > 0) {
      suggestions.push(
        `Show recent entries from ${focusTable.name} ordered by ${dateColumns[0].column_name}`
      );
    ...
    if (nameColumns.length > 0) {
      suggestions.push(
        `Find unique ${nameColumns[0].column_name} values in ${focusTable.name}`
      );
      suggestions.push(
        `Search ${focusTable.name} by ${nameColumns[0].column_name}`
      );
    }

    if (idColumns.length > 0) {
      suggestions.push(
        `Find specific record in ${focusTable.name} by ${idColumns[0].column_name}`
      );
    }
  }
  ...
  if (allTables.length > 1) {
    ...
    suggestions.push(
      `Compare record counts between ${
        focusTable?.name || allTables[0].name
      } and ${secondTable.name}`
    );
  }
